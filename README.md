...
| [`gateway/`](gateway/) | ENS CCIP-read (ENSIP-10) resolver gateway for `*.calibre.eth` — serves `addr()` + `text()` from the calibre public profile API | TypeScript |
| [`discord-bot/`](discord-bot/) | Role-sync bot: resolve members' subnames → read `gg.calibre.rank` → assign Discord role | TypeScript |
| [`ranking/`](ranking/) | Pure rank-bucketing lib (skill percentile → tier) + the canonical `gg.calibre.*` text-record key schema | Python |
